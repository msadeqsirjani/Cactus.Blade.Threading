...
namespace Cactus.Blade.Threading
{
    ...
    public class SoftLock
    {
        ...
        {
            return Interlocked.Exchange(ref _lock, LockAcquired) == LockNotAcquired;
        }
        ...
        public void Release()
        {
            Interlocked.Exchange(ref _lock, LockNotAcquired);
        }

        /// <summary>
        /// Gets a value indicating whether the lock has been acquired.
        /// </summary>
        public bool IsLockAcquired => _lock == LockAcquired;
    }
}
